...
class Searcher():
    def __init__(self, client):
        ...
        #client = pymongo.MongoClient('mongodb://server:3QWSD9MSMqwy6LvqkbTyP56URAMkgb@ec2-18-232-186-45.compute-1.amazonaws.com:27017/')
        self.webdb = client['webIndex']
        self.pagedb = client['pageIndex']
        self.logdb = client['logs']
        #update things that are older than 2 days
        self.updateAge = 172800
        #note when I switch up the architecture I will want to revisit this line to make sure N is up to date
        self.N = len(self.pagedb.list_collection_names())
        self.k_1 = 1.2
        self.k_2 = 100
        self.b = 0.75
        ...
        startTime = time.time()
        totalScore = 0
        for collection in self.pagedb.list_collection_names():
            document = self.pagedb[collection].find_one({'tag': 'totScore'})
            try: docAge = time.time()-document[lastUpdated]
            except: docAge = 0
            if type(document) == type(None) or docAge<self.updateAge:
                docScore = getTotalScore(collection, self.pagedb, self.weightDict)
                document = {'tag':'totScore', 'values': docScore, 'lastUpdated': time.time()}
                self.pagedb[collection].replace_one({'tag': 'totScore','values': docScore}, document, upsert = True)
            totalScore += document['values'] 
        self.avgds = totalScore/self.N
        print('avgerage doc score is ', self.avgds, 'and took ', time.time()-startTime)
        self.derankFactor = 0.1
